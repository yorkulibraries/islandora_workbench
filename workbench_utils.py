...
def issue_request(config, method, path, headers="", json="", data="", query={}):
    ...
    if method == "HEAD":
        response = requests.head(
            url, auth=(config["username"], config["password"]), headers=headers
        )
    if method == "POST":
...
def ping_islandora(config):
    ...
    try:
        ...
        message = (
            ...
            + config["host"]
            + ". Please check the hostname or network."
        )
        logging.error(message)
    ...
    if response.status_code == 200:
        ...
        if field_config["data"] == []:
            message = (
                config["host"]
                + " does not recognize the username/password combination you have provided."
            )
            logging.error(message)
            sys.exit("Error: " + message)
        else:
            print(
                "OK, "
                + config["host"]
                + " is accessible using the credentials provided."
            )
            logging.info(
                config["host"] + " is accessible using the credentials provided."
            )
...
def get_field_definitions(config):
    ...
    field_storage_config_url = (
        config["host"] + "/jsonapi/field_storage_config/field_storage_config"
    )
    field_storage_config_response = issue_request(
        config, "GET", field_storage_config_url, headers
    )
    if field_storage_config_response.status_code == 200:
        ...
        while "next" in field_storage_config["links"]:
            ...
            field_storage_config_response = issue_request(
                ...
                "",
                "",
                {"page[offset]": offset, "page[limit]": "50"},
            )
            field_storage_config = json.loads(field_storage_config_response.text)
            for item in field_storage_config["data"]:
                field_name = item["attributes"]["field_name"]
    ...
    if field_config_response.status_code == 200:
        ...
        for item in field_config["data"]:
            ...
            if field_name in field_definitions:
                ...
                if "config" in item["attributes"]["dependencies"]:
                    raw_vocabularies = [
                        x
                        for x in item["attributes"]["dependencies"]["config"]
        ...
        while "next" in field_config["links"]:
            offset = offset + 50
            field_config_response = issue_request(
                config,
                "GET",
                field_config_url,
                headers,
    ...
    base_field_override_url = (
        config["host"]
        + "/jsonapi/base_field_override/base_field_override?filter[type][condition][path]=bundle&filter[type][condition][value]="
        + config["content_type"]
    )
    base_field_override_response = issue_request(
        config, "GET", base_field_override_url, headers
    )
...
def check_input(config, args):
    ...
    if config["task"] == "add_media":
        ...
        if not set(config_keys) == set(add_media_options):
            message = (
                "Please check your config file for required values: "
                + joiner.join(add_media_options)
                + "."
            )
            logging.error(message)
            sys.exit("Error: " + message)
    if config["task"] == "delete_media":
        delete_media_options = [
            "task",
            "host",
            "username",
            "password",
        ...
        if not set(config_keys) == set(delete_media_options):
            message = (
                "Please check your config file for required values: "
                + joiner.join(delete_media_options)
                + "."
            )
            logging.error(message)
            sys.exit("Error: " + message)
    ...
    for count, row in enumerate(csv_data, start=1):
        ...
        if len(csv_column_headers) > string_field_count:
            logging.error(
                "Row %s of your CSV file does not "
        ...
        if len(csv_column_headers) < string_field_count:
            logging.error(
                "Row %s of your CSV file has more columns than there are headers "
                + "(%s).",
                str(count),
                str(string_field_count),
                str(len(csv_column_headers)),
    ...
    if config["task"] == "create":
        ...
        if config["id_field"] not in csv_column_headers:
            ...
            sys.exit("Error: " + message)
        if (
            "file" not in csv_column_headers
        ...
        if "parent_id" in csv_column_headers:
            if (
                "field_weight" not in csv_column_headers
                or "field_member_of" not in csv_column_headers
            ):
        ...
        for csv_column_header in csv_column_headers:
            if csv_column_header not in drupal_fieldnames:
                logging.error(
                    "CSV column header %s does not appear to match any Drupal field names.",
                    csv_column_header,
                )
                sys.exit(
    ...
    if config["task"] == "create":
        ...
        for drupal_fieldname in field_definitions:
            ...
            if (
                ...
                and field_definitions[drupal_fieldname]["entity_type"] == "node"
            ):
                if (
    ...
    if config["task"] == "update" or config["task"] == "create":
        ...
        if terms_view_response.status_code == 404:
            ...
            print(
                "Warning: Not validating taxonomy term IDs used in CSV file. To use this feature, install the Islandora Workbench Integration module."
            )
        else:
                ...
                for parent_nid in parent_nids:
                    if len(parent_nid) > 0:
                        ...
                        if parent_node_exists is False:
                            message = (
                                "The 'field_member_of' field in row "
                                + str(count)
    ...
    if (config["task"] == "create" or config["task"] == "add_media") and config[
        "paged_content_from_directories"
    ] is False:
            ...
            for count, file_check_row in enumerate(file_check_csv_data, start=1):
                ...
                if not os.path.exists(file_path) or not os.path.isfile(file_path):
                    message = (
                        "File "
                        + file_path
                        + ' identified in CSV "file" column for record with ID field value '
                        + file_check_row[config["id_field"]]
                        + " not found."
                    )
                    logging.error(message)
...
def create_media(config, filename, node_uri):
    ...
    media_headers = {"Content-Type": mimetype[0], "Content-Location": location}
    binary_data = open(os.path.join(config["input_dir"], filename), "rb")
    media_response = issue_request(
        config, "PUT", media_endpoint, media_headers, "", binary_data
    )
...
def create_term(config, vocab_id, term_name):
    ...
    term = {
        "vid": [{"target_id": vocab_id, "target_type": "taxonomy_vocabulary"}],
        "status": [{"value": True}],
        ...
        for field_name in field_max_lengths.keys():
            if field_name in row:
                ...
                for field_value in delimited_field_values:
                    ...
                    if (
                        field_name in field_max_lengths
                        and len(field_value) > field_max_lengths[field_name]
                    ):
                        if config["task"] == "create":
                            message = (
                                'CSV field "'
...
def validate_taxonomy_field_values(config, field_definitions, csv_data):
                    ...
                    if len(terms) == 0:
                        if config["allow_adding_terms"] is True:
                            ...
                            message = (
                                'Vocabulary "'
                                + vocabulary
                                + '" referenced in CSV field "'
                                + column_name
                                + '" may not be enabled in the "Terms in vocabulary" View. Please confirm it is.'
                            )
            ...
            if len(row[column_name]):
                ...
                for field_value in tids_to_check:
                    ...
                    if (
                        ...
                        and value_is_numeric(field_value) is not True
                    ):
                        ...
                        for split_field_value in split_field_values:
                            ...
                            if not namespaced:
                                message = (
                                    'Term names in multi-vocabulary CSV field "'
                                    + column_name
                                    + '" require a vocabulary namespace; value '
                                )
                                message_2 = (
                                    '"'
                                    + field_value
                    ...
                    if value_is_numeric(field_value):
                        if (
                            int(field_value)
                            not in fields_with_vocabularies[column_name]
                        ):
                            message = (
                                'CSV field "'
                                + column_name
                        ...
                        if value_is_numeric(tid) is not True:
                            ...
                            if len(this_fields_vocabularies) > 1:
                                ...
                                for split_field_value in split_field_values:
                                    ...
                                    [
                                        namespace_vocab_id,
                                        namespaced_term_name,
                                    ] = split_field_value.split(":")
                                    if (
                                        namespace_vocab_id
                                        not in this_fields_vocabularies
                                    ):
                                        message = (
                                            'CSV field "'
                                            + column_name
                                    ...
                                    if config["allow_adding_terms"] is True:
                                        ...
                                        if tid is False:
                                            ...
                                            message_2 = (
                                                'not in the referenced vocabulary ("'
                                                + namespace_vocab_id
                                                + '"). That tag will be added to that vocabulary.'
                                            )
                                            logging.warning(message + message_2)
                                            print("Warning: " + message + message_2)
                                    else:
    ...
    if new_term_names_in_csv is True and config["allow_adding_terms"] is True:
        print(
            "OK, term IDs/names in CSV file exist in their respective taxonomies (and new terms will be created as noted, but only once)."
        )
    else:
...
def create_children_from_directory(
    config, parent_csv_record, parent_node_id, parent_title
):
